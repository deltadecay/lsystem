...
function BranchLeaves({ length, radius, accScaleDown }) {
	...
	for (let i = 0; i < numLeaves; i++) {
		...
		vertices.push(x0, h, y0);
		vertices.push(
			(radius + leafLength) * vecx,
			h + leafWidth / 2.0,
			(radius + leafLength) * vecy,
		);
		vertices.push(
			(radius + leafLength) * vecx,
			h - leafWidth / 2.0,
			...
			{level <= 2 && (
				<group>
					<BranchLeaves
						length={h}
						radius={r}
						accScaleDown={accScaleDown}
					/>
				</group>
			)}

			{level > 1 &&
				branchAngles.map((branchAng, branchIdx) => {
					const branchDistR =
						branchIdx === 0 ? 0.97 : 0.5 + 0.47 * Math.random();
					const turnAngR = 20 + 40 * Math.random();
					const twistAngR =
						-(sectionAng / 2.0) + sectionAng * Math.random();
					return (
						<group
							key={"brang" + branchAng}
							rotation={[0, (branchAng * Math.PI) / 180.0, 0]}
							scale={scaleDown}
							position={[0, branchDistR * h, 0]}
						>
							<group
								rotation={[
									0,
									(twistAngR * Math.PI) / 180.0,
									(turnAngR * Math.PI) / 180.0,
								]}
							>
								<Tree
									level={level - 1}
									height={h}
									radius={r}
									accScaleDown={accScale * scaleDown}
								/>
							</group>
						</group>
					);
	...
	return (
		<div style={{ width: "75vw", height: "75vh" }}>
			...
			>
				...
				<ambientLight color={0xafafaf} intensity={1.5} />
				<directionalLight
					position={[5, 50, 5]}
					castShadow
				...
				<Center alignTop={false}>
					...
					{[
						[
							0, 0, 0,
						] /* [5,0,0], [-5,0,0], [0,0,-5], [0,0,5], [-5,0,5], [5,0,5], [-5,0,-5], [5,0,-5]*/,
					].map((pos, i) => {
						return (
							<Tree
								key={"Tree" + i}
